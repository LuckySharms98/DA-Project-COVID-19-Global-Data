...
--What countries have the highest infection rate compared to the population? Let's look at the latest date given for each country. 
...
--What's the breakdown of deaths per continent?
SELECT location, max(cast(total_deaths as INT)) AS max_deaths_figure
FROM CovidPortfolio..CovidDeaths
WHERE continent IS NULL AND location NOT LIKE 'European Union' AND location NOT LIKE '%income%'
GROUP BY location 
ORDER BY max_deaths_figure DESC;

--Let's take a look at the global figures. How do new cases and deaths evolve over time globally? Can we find a "peak time" for new cases/deaths? What about death rate? 
...
--Let's look at ALL cases in total. This is insane. 
SELECT SUM(cast(new_cases as INT)) AS total_new_cases, SUM(cast(new_deaths as INT)) AS total_new_deaths, SUM(cast(new_deaths as INT))/SUM(new_cases)*100 AS death_percentage  --this lets us find the sum # of said statistic across ALL locations on a given date
FROM CovidPortfolio..CovidDeaths
WHERE continent IS NOT NULL; 
